...
?>
  ...
  d+"px").css("background-color",a.isDark(e,i)?h.foreground:h.background).appendTo(f)}}a=c;jQuery(a).appendTo(this)})}})(jQuery);
    ...
    var server_seed_hash='<?php echo hash('sha256',$player['server_seed']); ?>';
    var client_seed='<?php echo $player['client_seed']; ?>';
    ...
    function generateNewAddress() {
      $("#fqrcode").html('<br><br><br><img src="content/images/ajax_loader.gif">');
      $.ajax({
    ...
    function _spincircle(d,spinData) {
      $.each($('audio'), function () {
        this.pause();
        this.currentTime=0;
        //this=0;
      });
      document.getElementById('audio-spinning').play();
      $("#wof-image").rotate({
        duration:5500,
        angle: 0, 
        animateTo:d,
        callback: function() {
          rewriteStatuses(spinData);
        }
      });
    }
    function deposit() {
      $.msgBox({
        title:"Deposit Funds",
        content:"<br><small>Send <b>minimum 0.00000001</b> <?php echo $settings['currency_sign']; ?> to your unique deposit address:</small><br><br><div id=\"_dp_addr\"></div><div id=\"fqrcode\" style=\"margin-top:5px;margin-left:60px;width:250px;height:250px;\"></div><br><small><small><a class=\"microbuttons\" href=\"#\" onclick=\"javascript:generateNewAddress();return false;\">Generate new address</a> <a href=\"#\" onclick=\"javascript:generateNewAddress();return false;\"><img src=\"content/images/refresh.png\" style=\"width: 10px; height: 10px;\"></a> | <a class=\"microbuttons\" href=\"#\" onclick=\"javascript:return viewPending();\">Show pending deposits</a> <a href=\"#\" onclick=\"javascript:return viewPending();\"><img src=\"content/images/pending.png\" style=\"width: 10px; height: 10px;\"></a></small></small><br><br><small><small><i><b>Warning:</b> This address is just for a single use. If you want to deposit multiple times, you should generate new address!</i></small></small>",
        type:"info",
        opacity:0.8,
        buttons: [{ value: "Close" }],        
    ...
    function rewriteStatuses(data) {
      if (data['spin']=='spin' && data['data']!='0') refreshbalance();
      if (data['data']=='0.25') dataData='<span class="afterDot">.25</span>';
      else if (data['data']=='1.25') dataData='1<span class="afterDot">.25</span>';
      else if (data['data']=='1.5') dataData='1<span class="afterDot">.5</span>';
      else dataData=data['data'];
      // play sound
      $.each($('audio'), function () {
        this.pause();
        this.currentTime=0;
      ...
      if (parseFloat(data['data'])>1)
        document.getElementById('audio-success').play();      
      else
        document.getElementById('audio-lose').play();
            
      $("#middler").html(dataData+'<span class="nob">x</span>');
      $("#yspins").html(data['tspins']);
      $("#tspins").html(data['tspins_']);
      spinningNow=false;
    }
    function spin() {
      if (!spinningNow) {
        $("#middler").html('');
        $.ajax({
          'url': './content/ajax/spin.php?_unique=<?php echo $unique; ?>&w='+$("#bet_input").val()+'&client_seed='+client_seed,
          'dataType': "json",
          'success': function(data) {
            if (data['error']=='yes') {
              if (data['data']=='invalid_bet') al='Invalid bet!';
              else if (data['data']=='too_small') al='Too small!';
              else if (data['data']=='too_big_bet') al='You\'ve entered too big bet. At this time we only accept bets which are not bigger than '+data['under']+' BTC.';
              alert(al);
            }
            else {
              spinTo=parseFloat(data['spinTo'])+720;
              spinningNow=true;
              $("#balance_").html(data['reduceTo']);
              _spincircle(spinTo,data);              
            } 
          }
        });
      }
    }
    function _fillPercents(p) {
      $.ajax({
        'url': './content/ajax/request_balance.php?_unique=<?php echo $unique; ?>',
        'dataType': "json",
        'success': function(data) {
          if (p==100) s=(data['balance']);
          if (p==50) s=(data['balance']/2);
          if (p==10) s=(data['balance']/10);
          s=s.toString().split('.');
          $("#bet_input").val(s[0]+'.'+s[1].substring(0,10).replace(/0*$/,''));
        }
      });
      return false;
    }
    function _runEnter(e) {
      if (e.keyCode==13) {
        spin();
        return false;
      }
    ...
    function tm_interval_content_(con) {
      if (spinningNow==false) { 
        $.ajax({
          'url': './content/ajax/_stats_load.php?con='+con+'&_unique=<?php echo $unique; ?>',
          'dataType': "json",
          'success': function(data) {
            $("div#stats-content").html(data['content']);
            $("ul#stats-menu a.active").removeClass('active');
            $("ul#stats-menu a#stats_"+con).addClass('active');          
          }
        });
      }
      timeout_=setTimeout("tm_interval_content_('"+con+"')",3000);
    }
    ...
    function call_randomize() {
      cl_seed=$("#cl_seed").val();
      client_seed=cl_seed;
      $("#sranec_").html('<br><img src="content/images/ajax_loader.gif">');
      $.ajax({
        'url': './content/ajax/randomize.php?_unique=<?php echo $unique; ?>&client_seed='+cl_seed,
        'dataType': "json",
        'success': function(data) {
          $("#sranec_").html('<br>Your client seed for the next spin was changed!<br><br><b>Client seed:</b> <input type=\"text\" id=\"cl_seed\" width=\"60%\" value=\""+generate_cl_seed()+"\"><br><button style=\"padding: 3px;\" onclick=\"javascript:call_randomize();\">Set client seed</button>');
        }
      });
    }
    function generate_cl_seed() {
      return Math.floor((Math.random()*999999999999999)+1);
    }
    function randomize() {
      $.msgBox({
        title:"Randomize",
        content:"<div id=\"sranec_\"><br><b>Client seed:</b> <input type=\"text\" id=\"cl_seed\" width=\"60%\" value=\""+generate_cl_seed()+"\"><br><button style=\"padding: 3px;\" onclick=\"javascript:call_randomize();\">Set client seed</button></div>",
        type:"warning",
        opacity:0.8,
        buttons: [{ value: "Close" }]        
      });
